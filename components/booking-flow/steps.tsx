...
export function BookingFlow({
  ...
  useEffect(() => {
    ...
    const initializeFromParams = async () => {
      try {
        ...
        const checkInParam = searchParams.get("checkIn") || searchParams.get("check_in")
        const checkOutParam = searchParams.get("checkOut") || searchParams.get("check_out")
        const adultsParam = searchParams.get("adults")
        const childrenParam = searchParams.get("children")
        const roomIdParam = searchParams.get("roomId") || searchParams.get("room_id")

        // Set dates if provided in URL
        if (checkInParam && !initialDates.checkIn) {
          setCheckIn(new Date(checkInParam))
        }

        if (checkOutParam && !initialDates.checkOut) {
          setCheckOut(new Date(checkOutParam))
        }

        // Set guests if provided in URL
        if (adultsParam && !initialGuests.adults) {
          setAdults(adultsParam)
        }

        if (childrenParam && !initialGuests.children) {
          setChildren(childrenParam)
        }

        // If we have a roomId, fetch that room
        if (roomIdParam && !initialRoomId) {
          await fetchRoom(roomIdParam)
        } else if (initialRoomId) {
          await fetchRoom(initialRoomId)
        }

        // If we have dates, check availability
        if ((checkIn || initialDates.checkIn) && (checkOut || initialDates.checkOut)) {
          await checkAvailability(checkIn || initialDates.checkIn!, checkOut || initialDates.checkOut!)
        }
      } catch (error) {
  ...
  }, [searchParams, supabase, initialDates, initialGuests, initialRoomId])
    ...
    try {
      ...
      if (isUUID) {
        ...
        if (room) {
          ...
          if (currentStep === 1) setCurrentStep(3) // Skip to room selection step
        }
      } else {
        // Try to find by name or other property
        const { data: rooms, error } = await supabase.from("rooms").select("*")

        if (error) throw error

        if (rooms && rooms.length > 0) {
          const room = rooms.find((r) => r.name.toLowerCase() === roomId.toLowerCase() || r.slug === roomId)
  ...
  const checkAvailability = async (startDate: Date, endDate: Date) => {
    ...
    try {
      ...
      // If we already have a selected room, check if it's available
      if (selectedRoom) {
